...
static void ZMIPSetPixel(MipMap pixmap,MipMap zbuffer,
	...
	for(i=0;i<MAX_MIP_PLANES;i++)
		...
		oldC=getPixelColor(pixmap.plane[i],x,y);
		//printf("%d:%d,%d:%f,%f\n",i,x,y,z,oldZ);
		if(z<oldZ)
				{
				...
				if(pixmap.plane[i].data)
					setPixelColor(pixmap.plane[i],x,y, C);
	...
	for(;p>0;p--)
		...
		for(x=0;x<m.plane[p-1].width;x++)
			...
			for(y=0;y<m.plane[p-1].height;y++)
				{
				NCCAPixel D={0,0,0,0};
				NCCAPixel C=getPixelColor(m.plane[p-1],x,y);
				float topZ,bottomZ;
				int scale=0;
				...
				topZ=getPixelGrey(z.plane[p-1],x,y);
				
				bottomZ=getPixelGrey(z.plane[p],x>>1,y>>1);
				if(bottomZ>topZ)
					{
					NCCAPixel tmp=getPixelColor(m.plane[p],x>>1,y>>1);
					D=pixelSum(D,tmp);
					scale++;
					}
				bottomZ=getPixelGrey(z.plane[p],(x+1)>>1,(y)>>1);
				if(bottomZ>topZ)
					{
					NCCAPixel tmp=getPixelColor(m.plane[p],(x+1)>>1,(y)>>1);
					D=pixelSum(D,tmp);
					scale++;
					}
				bottomZ=getPixelGrey(z.plane[p],(x)>>1,(y+1)>>1);
				if(bottomZ>topZ)
					{
					NCCAPixel tmp=getPixelColor(m.plane[p],(x)>>1,(y+1)>>1);
					D=pixelSum(D,tmp);
					scale++;
					}
				bottomZ=getPixelGrey(z.plane[p],(x+1)>>1,(y+1)>>1);
				if(bottomZ>topZ)
					{
					NCCAPixel tmp=getPixelColor(m.plane[p],(x+1)>>1,(y+1)>>1);
					D=pixelSum(D,tmp);
					scale++;
					}
				if(scale==0)
				...
				C.b+=(1-C.a)*D.b/scale;
				C.a+=(1-C.a)*D.a/scale;
				setPixelColor(m.plane[p-1],x,y,C);
				}
			}
		}
	}
